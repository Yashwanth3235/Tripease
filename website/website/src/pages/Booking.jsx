...
import { FaPlane, FaArrowLeft, FaCreditCard, FaLock, FaShieldAlt, FaCheckCircle } from 'react-icons/fa';
...
const Booking = () => {
        ...
        const response = await fetch('http://localhost:8080/api/bookings/book', {
            ...
            headers: {
             ...
             body: JSON.stringify({
                ...
                from: searchData.from,     // 🔥 changed
                to: searchData.to,         // 🔥 changed
                date: searchData.date,     // 🔥 changed
                price: Number(flight.price) * (searchData.passengers || 1)
            })
        });
    ...
    const completeBooking = async () => {
        ...
        const tripData = {
            title: `Trip to ${searchData.to}`,
            destination: searchData.to,
            startDate: searchData.date,
            endDate: searchData.date,
            budget: flight.price * (searchData.passengers || 1),
            category: 'booking',
        ...
        try {
            // ✅ FIXED: Sending correct structure expected by backend
            const response = await fetch('http://localhost:8080/api/flights/book', {
                method: 'POST',
            ...
            alert(`Payment Successful! Booking confirmed for ${passengers.length} passenger(s). Ticket details have been sent to ${passengers[0].email}.`);
            navigate(`/booking-confirmation/${newTrip.id}`);
        } catch (error) {
            console.error('Email sending error:', error);
            alert(`Payment Successful! Booking confirmed, but we couldn't send the email ticket.`);
            navigate(`/booking-confirmation/${newTrip.id}`);
        }
    };
                    ...
                    <Col lg={4}>
                        <Card className="glass-panel border-0 p-4 sticky-top shadow-sm" style={{ top: '90px', zIndex: 10 }}>
                            ...
                            <div className="d-flex justify-content-between mb-4">
                                ...
                                <h5 className="fw-bold mb-0 text-primary">₹{flight.price * searchData.passengers}</h5>
                            </div>
                            ...
                            <Card className="bg-dark border-secondary p-3 rounded-3">
                                <div className="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <small className="text-muted">{flight.flightNum}</small>
                                    </div>
                                </div>
                                <div className="mt-3 d-flex justify-content-between small">
                                    <span>{searchData.fromCode}</span>
                                    <span className="text-muted">{flight.dep}</span>
                                    <span>→</span>
                                    <span>{searchData.toCode}</span>
                                    <span className="text-muted">{flight.arr}</span>
                                </div>
                            </Card>
                        </Card>
                    </Col>
                ...
                <Modal.Body className="p-0 bg-white text-dark">
                    ...
                    <div className="p-4 d-flex justify-content-between align-items-center border-bottom" style={{ background: '#ffffff', color: '#000000' }}>
                        ...
                        <div className="text-end" style={{ color: '#000000' }}>
                            <div className="h4 mb-0 fw-bold">₹{flight.price * searchData.passengers}</div>
                    ...
                    <div className="p-4 bg-white">
                            ...
                            >
                                ...
                                <div className="mt-4">
                                    <Button
                                        ...
                                        onClick={handleProcessPayment}
                                    >
                                        PAY ₹{flight.price * searchData.passengers}
                                    </Button>
                        ...
                        {paymentStatus === 'success' && (
                            ...
                            >
                                ...
                                <p style={{ color: '#666666', fontSize: '12px' }}>Your booking is being generated...</p>
                            </motion.div>
                        )}
                    </div>
